...
1. Push this project to its own GitHub repo (`aida-autopilot`)
...
4. Set variables (service → Variables): everything in `.env.example`.
   Leave `AIDA_SHADOW_MODE=true` for the shadow phase. Railway injects `PORT` itself.
5. Note the public URL Railway assigns, then configure the two Halo webhooks against it:
   - `https://<railway-url>/webhooks/halo` with header `X-Aida-Token: <AIDA_WEBHOOK_SECRET>`
...
- A backup job for the `aida-data` volume (single small SQLite file)

```bash
git clone <repo> && cd aida-autopilot
...
4. **Move scheduler state** — either:
   - *Clean*: stop the Railway service, copy `/data/autopilot.db` from the Railway volume
...
- **Backups**: `/data/autopilot.db` is everything the service owns; tickets themselves are
  safe in Halo regardless
- **Monitoring**: poll `/health` from your RMM/monitoring; alert on any integration ≠ `ok`
- **Going live** is one variable: `AIDA_SHADOW_MODE=false` + restart — do this per the
